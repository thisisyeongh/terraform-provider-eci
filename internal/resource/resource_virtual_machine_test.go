...
func TestAccVirtualMachine_basic(t *testing.T) {
	...
	if instanceTypeName == "" || pricingName == "" || pricingType == "" {
		t.Skip(
			"ECI_TEST_INSTANCE_TYPE_NAME, ECI_TEST_VM_PRICING_NAME, and ECI_TEST_VM_PRICING_TYPE must be set",
		)
	}
...
func testAccCheckVirtualMachineDestroy(s *terraform.State) error {
	...
	for _, rs := range s.RootModule().Resources {
		...
		return fmt.Errorf(
			"virtual machine %s still exists (status: %s)",
			rs.Primary.ID,
			resp.Status,
		)
	}
...
func testAccVirtualMachineConfig_basic(
	rName, instanceTypeName, pricingName, pricingType string,
) string {
