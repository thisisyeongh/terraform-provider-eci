...
func TestAccBlockStorageSnapshot_basic(t *testing.T) {
	...
	if instanceTypeName == "" || vmPricingName == "" || vmPricingType == "" ||
		...
		t.Skip("Required ECI_TEST_* env vars not set for block storage snapshot test")
	}
	...
	resource.ParallelTest(t, resource.TestCase{
		...
		Steps: []resource.TestStep{
			{
				Config: testAccBlockStorageSnapshotConfig_basic(
					rName, instanceTypeName, vmPricingName, vmPricingType,
					bsPricingName, bsPricingType, imageName,
				),
				Check: resource.ComposeAggregateTestCheckFunc(
					resource.TestCheckResourceAttrSet("eci_block_storage_snapshot.test", "id"),
					resource.TestCheckResourceAttr("eci_block_storage_snapshot.test", "name", rName),
					resource.TestCheckResourceAttrSet("eci_block_storage_snapshot.test", "block_storage_id"),
				),
			},
		},
	})
...
func testAccCheckBlockStorageSnapshotDestroy(s *terraform.State) error {
	...
	for _, rs := range s.RootModule().Resources {
		if rs.Type != "eci_block_storage_snapshot" {
			continue
		}

		resp, err := client.GetBlockStorageSnapshot(rs.Primary.ID)
		if err != nil {
		...
		return fmt.Errorf("block storage snapshot %s still exists (status: %s)", rs.Primary.ID, resp.Status)
	}
	return nil
}

func testAccBlockStorageSnapshotConfig_basic(
	rName, instanceTypeName, vmPricingName, vmPricingType,
...
resource "eci_block_storage_snapshot" "test" {
  name             = %[1]q
  block_storage_id = eci_block_storage.test.id
  tags             = {}
}
`, rName, instanceTypeName, vmPricingName, vmPricingType,
